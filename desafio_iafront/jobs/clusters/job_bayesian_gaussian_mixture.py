...
from desafio_iafront.jobs.clusters.clusters import bayesian_gaussian_mixture
...
def main(dataset: str, number_of_cluster: int, saida: str, data_inicial, data_final):
    ...
    labels = bayesian_gaussian_mixture(vector, number_of_cluster)

    dataset['cluster_label'] = list(labels)

    saida = os.path.join(saida, "bayesian_gaussian_mixture")
    os.mkdir(saida)
